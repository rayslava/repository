...
[[ ${PV} = *9999* ]] && CVS_ECLASS="git-2" || CVS_ECLASS=""

inherit common-lisp-3 git-2 elisp eutils
...
src_compile() {
	...
	BYTECOMPFLAGS="${BYTECOMPFLAGS} -L lib -l slime" \
		elisp-compile lib/*.el || die
...
src_install() {
	...
	common-lisp-install-sources swank/*.lisp
	...
	## install libs
	elisp-install ${PN}/lib/ lib/*.el \
		|| die "Cannot install libs"

	## install docs
	dodoc README* ChangeLog NEWS PROBLEMS
